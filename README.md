...
`clijp` は同じ入力に対する翻訳結果をローカルにキャッシュします。これにより同一コマンドを何度も翻訳する際の待ち時間と Copilot の呼び出し回数を削減できます。

- キャッシュディレクトリ: `~/.cache/clijp`
- キャッシュキー: 入力テキストの SHA256 ハッシュ（ファイル名は `<hash>.txt`）
- 挙動:
  - キャッシュが存在する場合は Copilot を呼ばず、キャッシュ内容を出力します（高速パス）。
  - キャッシュが存在しない場合は Copilot に問い合わせ、取得した翻訳をキャッシュに書き込みます（書き込みはベストエフォート）。
